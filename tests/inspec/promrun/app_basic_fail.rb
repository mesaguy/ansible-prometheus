...
describe file('/etc/prometheus/node_exporter_textfiles/promrun_app_fail.prom') do
  it { should_not exist }
end
...
describe file('/etc/prometheus/node_exporter_textfiles/promrun_app_fail.prom') do
  ...
  its('content') { should match /promrun_app_fail_starttime\{user="app"\} [0-9]+/ }
  ...
  its('content') { should match /promrun_app_fail_exit_status\{user="app"\} [1-2]/ }
  ...
  its('content') { should match /# HELP promrun_app_fail_endtime Start time in Unix time with microseconds./ }
  its('content') { should match /# TYPE promrun_app_fail_endtime gauge/ }
  its('content') { should match /promrun_app_fail_endtime\{user="app"\} [0-9]+/ }
  its('size') { should > 200 }
